...
// One-time auth flow for Google AdSense Management API.
// Run: node scripts/audit/adsense-auth.js
// Reuses .gsc-oauth.json for client credentials (same Google Cloud project).
// Saves refresh token to .adsense-token.json (gitignored).
//
// Prereq: enable AdSense Management API for the same Google Cloud project:
...
const TOKEN_PATH = path.join(ROOT, '.adsense-token.json');
...
async function main() {
  if (!fs.existsSync(OAUTH_PATH)) {
    console.error(`Missing ${OAUTH_PATH}. Run gsc-auth.js first or place your OAuth Desktop client JSON there.`);
    process.exit(1);
  }
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (!tokens.refresh_token) {
        ...
        console.error('\nNo refresh token. Revoke this client at https://myaccount.google.com/permissions and rerun.\n');
        server.close();
        process.exit(1);
      }
      ...
      res.end('<html><body style="font-family:system-ui;padding:40px;"><h1>Authenticated</h1><p>AdSense token saved. You can close this tab.</p></body></html>');
  ...
  server.listen(PORT, () => {
    ...
    console.log('Opening browser for Google sign-in (use the account that owns AdSense)...\n');
    openBrowser(authUrl);
  });
}
